...
sigma_MLE = 1/n_echantillon*((X(I_0,1:2)-mu_0_MLE).'*(X(I_0,1:2)-mu_0_MLE)+(X(I_1,1:2)-mu_1_MLE).'*(X(I_1,1:2)-mu_1_MLE));
...
%Plotting p(y=1|x)
syms x y
ezplot(p_1(x,y,Pi_MLE,mu_0_MLE,mu_1_MLE,sigma_MLE)==0.5)

hold off

title(file);

print(fg,strcat('lda/',file),'-dpdf','-r0')
